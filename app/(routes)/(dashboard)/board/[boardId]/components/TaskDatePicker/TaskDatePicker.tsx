...
export function TaskDatePicker({ taskId, listId, startDate, dueDate }: TaskDatePickerProps) {
  ...
  const [dueHour, setDueHour] = useState(dueDate ? String(dueDate.getHours()).padStart(2, "0") : "23");
  const [dueMinute, setDueMinute] = useState(dueDate ? String(dueDate.getMinutes()).padStart(2, "0") : "59");
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        {hasDate ? (
          <span>
            ...
            {dueEnabled && due ? format(buildDueWithTime(due)!, "d MMM yyyy HH:mm", { locale: es }) : ""}
          </span>
        ) : (
          <span>Fechas</span>
        )}
      ...
      {open && (
        <div className="flex flex-col gap-4 rounded-xl border bg-popover p-4 shadow-md w-fit">
