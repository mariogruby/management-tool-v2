...
export function TaskLabels({ taskId, boardId, activeLabels }: TaskLabelsProps) {
  ...
  const deleteLabel = async (labelId: string) => {
    ...
    setActiveIds((prev) => { const n = new Set(prev); n.delete(labelId); return n; });
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        {activeIds.size > 0 && (
          <div className="flex gap-1">
            {labels.filter((l) => activeIds.has(l.id)).map((l) => (
              <span
                key={l.id}
      ...
      {open && (
        <div className="flex flex-col gap-2 rounded-xl border bg-popover p-3 shadow-md w-64">
          {!showForm ? (
            <>
              <p className="text-xs font-medium text-muted-foreground px-1">Etiquetas</p>
                ...
                {labels.map((label) => (
                  <div key={label.id} className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-2 flex-1 rounded-md px-2 py-1.5 hover:bg-muted transition-colors text-sm"
                    >
                      <span
                        className="h-6 flex-1 rounded-sm text-xs text-white px-2 flex items-center font-medium"
            ...
            <>
              ...
              {editingLabel && (
                <button
                  onClick={() => { deleteLabel(editingLabel.id); setShowForm(false); }}
